...
public class ApigeeLlmTest {
  ...
  @Test
  public void build_withRetryOptions_setsRetryOptionsInHttpOptions() {
    HttpRetryOptions retryOptions = HttpRetryOptions.builder().attempts(5).build();
    ApigeeLlm llm =
        ApigeeLlm.builder()
            .modelName("apigee/gemini-1.5-flash")
            .proxyUrl(PROXY_URL)
            ...
            .build();
    assertThat(llm.getHttpOptions().retryOptions()).hasValue(retryOptions);
  }
  ...
  public void build_withTrailingSlashInModel_parsesVersionAndModelId() {
    ...
    assertThat(requestCaptor.getValue().model()).hasValue("");

    llm = ApigeeLlm.builder().modelName("apigee/gemini/v1/").proxyUrl(PROXY_URL).build();
    assertThat(llm.getHttpOptions().apiVersion()).hasValue("v1");
  }
